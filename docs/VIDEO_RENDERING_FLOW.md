# 视频流渲染到VR的完整流程

## 📊 系统架构概览

```
┌─────────────────────────────────────────────────────────────────┐
│                      VR 虚拟机器人系统                            │
└─────────────────────────────────────────────────────────────────┘

服务器端 (Python)                          客户端 (JavaScript)
┌──────────────────────┐                  ┌──────────────────────┐
│  PyBullet 物理仿真    │                  │   WebXR VR 场景      │
│  ┌────────────────┐  │                  │  ┌────────────────┐  │
│  │ 虚拟机器人      │  │                  │  │  Three.js      │  │
│  │ - 人形机器人    │  │                  │  │  - VR 相机     │  │
│  │ - 彩色立方体    │  │                  │  │  - 视频屏幕    │  │
│  │ - 地面         │  │                  │  │  - 手柄        │  │
│  └────────────────┘  │                  │  └────────────────┘  │
│         ↓            │                  │         ↑            │
│  ┌────────────────┐  │                  │  ┌────────────────┐  │
│  │ 双目相机渲染    │  │                  │  │ Shader 拆分    │  │
│  │ - 左眼图像      │  │                  │  │ - 左眼纹理     │  │
│  │ - 右眼图像      │  │                  │  │ - 右眼纹理     │  │
│  └────────────────┘  │                  │  └────────────────┘  │
│         ↓            │                  │         ↑            │
│  ┌────────────────┐  │   WebRTC P2P    │  ┌────────────────┐  │
│  │ Side-by-Side   │──┼─────────────────┼─→│ 视频流接收      │  │
│  │ 视频拼接        │  │   (单轨道)       │  │                │  │
│  └────────────────┘  │                  │  └────────────────┘  │
│         ↓            │                  │         ↑            │
│  ┌────────────────┐  │                  │  ┌────────────────┐  │
│  │ WebRTC 编码    │  │                  │  │ WebRTC 解码    │  │
│  └────────────────┘  │                  │  └────────────────┘  │
└──────────────────────┘                  └──────────────────────┘
         ↓                                          ↑
         └──────────────── 网络传输 ─────────────────┘
```

---

## 🎬 视频渲染流程详解

### 第一步：PyBullet 场景渲染（服务器端）

#### 1.1 场景内容（`robot_sim.py`）

**虚拟机器人场景包含：**

```python
# 1. 地面
self.plane_id = p.loadURDF("plane.urdf")

# 2. 人形机器人（humanoid）
self.robot_id = p.loadURDF(
    "humanoid/humanoid.urdf",
    [0, 0, 0.9],  # 初始位置：离地面 0.9 米
    useFixedBase=False
)

# 3. 四个彩色立方体（环绕机器人）
cube_positions = [
    [1, 0, 0.5],   # 前方 - 红色
    [0, 1, 0.5],   # 左侧 - 绿色
    [-1, 0, 0.5],  # 后方 - 蓝色
    [0, -1, 0.5]   # 右侧 - 黄色
]
```

**场景布局：**
```
        绿色立方体
            │
            │
蓝色立方体 ─┼─ 机器人 ─ 红色立方体
            │
            │
        黄色立方体
            │
        ───地面───
```

#### 1.2 双目相机渲染（`stereo_camera.py`）

**关键参数：**
- 分辨率：640×480（每只眼睛）
- 视场角（FOV）：90°
- 瞳距（IPD）：64mm（模拟人眼间距）

**渲染过程：**

```python
def render_stereo(self):
    # 1. 获取机器人头部位置和朝向
    head_pos, head_orn = self.robot_sim.get_head_pose()
    
    # 2. 将四元数转换为旋转矩阵
    rotation_matrix = p.getMatrixFromQuaternion(head_orn)
    
    # 3. 计算方向向量
    forward = rotation_matrix @ [1, 0, 0]  # 前方
    up = rotation_matrix @ [0, 0, 1]       # 上方
    right = rotation_matrix @ [0, -1, 0]   # 右方
    
    # 4. 计算左眼位置（向左偏移 32mm）
    left_eye_pos = head_pos - right * 0.032
    
    # 5. 计算右眼位置（向右偏移 32mm）
    right_eye_pos = head_pos + right * 0.032
    
    # 6. 渲染左眼图像
    left_img = p.getCameraImage(
        width=640, height=480,
        viewMatrix=left_view_matrix,
        projectionMatrix=self.projection_matrix
    )
    
    # 7. 渲染右眼图像
    right_img = p.getCameraImage(
        width=640, height=480,
        viewMatrix=right_view_matrix,
        projectionMatrix=self.projection_matrix
    )
    
    return left_img, right_img
```

**立体视觉原理：**
```
        左眼视角          右眼视角
           ↓                ↓
    ┌──────────┐      ┌──────────┐
    │  🟥      │      │      🟥  │  ← 红色立方体
    │          │      │          │    （视差产生深度感）
    │    🤖    │      │    🤖    │  ← 机器人
    │          │      │          │
    └──────────┘      └──────────┘
     640×480           640×480
```

---

### 第二步：Side-by-Side 拼接（服务器端）

#### 2.1 图像拼接（`stereo_camera.py`）

```python
def render_stereo_sbs(self):
    # 1. 渲染左右眼图像
    left_img, right_img = self.render_stereo()
    
    # 2. 水平拼接
    sbs_img = np.hstack([left_img, right_img])
    
    # 输出尺寸：1280×480
    return sbs_img
```

**拼接结果：**
```
┌─────────────────────────────────────┐
│  左眼图像 (640×480) │ 右眼图像 (640×480) │
│                    │                    │
│      🟥            │            🟥      │
│                    │                    │
│        🤖          │          🤖        │
│                    │                    │
└─────────────────────────────────────┘
        Side-by-Side 格式 (1280×480)
```

---

### 第三步：WebRTC 视频传输

#### 3.1 视频编码（`webrtc_server.py`）

```python
class RobotVideoTrack(VideoStreamTrack):
    async def recv(self):
        # 1. 渲染 Side-by-Side 图像
        sbs_img = self.camera.render_stereo_sbs()
        
        # 2. 转换为 VideoFrame
        frame = VideoFrame.from_ndarray(sbs_img, format='bgr24')
        frame.pts = self.pts
        frame.time_base = fractions.Fraction(1, 1000000)
        
        # 3. 返回给 WebRTC 编码器
        return frame
```

#### 3.2 网络传输

- **协议**：WebRTC P2P
- **编码**：VP8/VP9/H.264（浏览器自动选择）
- **传输**：单个视频轨道（1280×480）
- **延迟**：< 100ms

---

### 第四步：客户端接收和解码

#### 4.1 WebRTC 接收（`webrtc-client.js`）

```javascript
pc.ontrack = (event) => {
    const stream = event.streams[0];
    
    // Side-by-Side 模式：只接收一个视频轨道
    if (this.videoMode === 'sbs') {
        this.onVideoTrack(stream);
    }
};
```

#### 4.2 创建视频元素（`vr-scene.js`）

```javascript
setupStereoVideoSBS(stream) {
    // 1. 创建 HTML5 视频元素
    const video = document.createElement('video');
    video.srcObject = stream;
    video.autoplay = true;
    video.play();
    
    // 2. 创建 Three.js 视频纹理
    const videoTexture = new THREE.VideoTexture(video);
    
    // 视频纹理包含完整的 Side-by-Side 图像 (1280×480)
}
```

---

### 第五步：Shader 拆分和渲染

#### 5.1 Shader 拆分（`stereo-shader.js`）

**核心原理：通过 UV 坐标映射，从单个纹理中采样不同区域**

```glsl
// Fragment Shader
uniform sampler2D videoTexture;  // 完整的 SBS 纹理 (1280×480)
uniform int eyeIndex;            // 0=左眼, 1=右眼

varying vec2 vUv;  // UV 坐标 (0.0 ~ 1.0)

void main() {
    vec2 uv = vUv;
    
    if (eyeIndex == 0) {
        // 左眼：采样左半部分
        // 将 U 坐标从 [0.0, 1.0] 映射到 [0.0, 0.5]
        uv.x = uv.x * 0.5;
    } else {
        // 右眼：采样右半部分
        // 将 U 坐标从 [0.0, 1.0] 映射到 [0.5, 1.0]
        uv.x = uv.x * 0.5 + 0.5;
    }
    
    gl_FragColor = texture2D(videoTexture, uv);
}
```

**UV 映射示意图：**
```
Side-by-Side 纹理 (1280×480)
┌─────────────────────────────────────┐
│  左眼 (0.0~0.5)  │  右眼 (0.5~1.0)  │
│                  │                  │
│      🟥          │          🟥      │
│                  │                  │
└─────────────────────────────────────┘
  U: 0.0    0.5    0.5           1.0

左眼 Shader:                右眼 Shader:
uv.x = uv.x * 0.5          uv.x = uv.x * 0.5 + 0.5
采样 [0.0, 0.5]            采样 [0.5, 1.0]
```

#### 5.2 创建双屏幕（`vr-scene.js`）

```javascript
// 左眼屏幕
const leftMaterial = new THREE.ShaderMaterial({
    uniforms: {
        videoTexture: { value: videoTexture },
        eyeIndex: { value: 0 }  // 左眼
    },
    vertexShader: StereoVideoShaderSBS.vertexShader,
    fragmentShader: StereoVideoShaderSBS.fragmentShader
});

this.leftScreen = new THREE.Mesh(geometry, leftMaterial);
this.leftScreen.layers.set(1);  // 图层 1
this.scene.add(this.leftScreen);

// 右眼屏幕
const rightMaterial = new THREE.ShaderMaterial({
    uniforms: {
        videoTexture: { value: videoTexture },
        eyeIndex: { value: 1 }  // 右眼
    },
    vertexShader: StereoVideoShaderSBS.vertexShader,
    fragmentShader: StereoVideoShaderSBS.fragmentShader
});

this.rightScreen = new THREE.Mesh(geometry, rightMaterial);
this.rightScreen.layers.set(2);  // 图层 2
this.scene.add(this.rightScreen);
```

---

### 第六步：Three.js 图层系统

#### 6.1 图层配置（`vr-scene.js`）

**Three.js 图层系统原理：**
- Three.js 支持 32 个图层（0-31）
- 每个物体可以属于一个或多个图层
- 每个相机可以选择渲染哪些图层

**WebXR 相机结构：**
```
xrCamera (VRCamera - 父相机)
├── cameras[0] (左眼子相机)
└── cameras[1] (右眼子相机)
```

**配置代码：**
```javascript
configureStereoLayers() {
    const xrCamera = this.renderer.xr.getCamera();
    
    // 1. 配置父相机：启用所有图层
    xrCamera.layers.enable(0);  // 场景物体
    xrCamera.layers.enable(1);  // 左眼视频
    xrCamera.layers.enable(2);  // 右眼视频
    
    // 2. 配置左眼相机：图层 0 + 1
    xrCamera.cameras[0].layers.disableAll();
    xrCamera.cameras[0].layers.enable(0);
    xrCamera.cameras[0].layers.enable(1);
    
    // 3. 配置右眼相机：图层 0 + 2
    xrCamera.cameras[1].layers.disableAll();
    xrCamera.cameras[1].layers.enable(0);
    xrCamera.cameras[1].layers.enable(2);
}
```

**渲染结果：**
```
左眼看到：                    右眼看到：
┌──────────────┐            ┌──────────────┐
│ 图层 0 (场景) │            │ 图层 0 (场景) │
│ +            │            │ +            │
│ 图层 1 (左眼) │            │ 图层 2 (右眼) │
│              │            │              │
│   🟥         │            │         🟥   │
│              │            │              │
│     🤖       │            │       🤖     │
│              │            │              │
└──────────────┘            └──────────────┘
```

---

## 🎯 完整数据流

```
1. PyBullet 渲染
   ↓
   左眼图像 (640×480, BGR)
   右眼图像 (640×480, BGR)

2. Side-by-Side 拼接
   ↓
   SBS 图像 (1280×480, BGR)

3. WebRTC 编码
   ↓
   视频流 (VP8/H.264)

4. 网络传输
   ↓
   P2P 连接 (< 100ms)

5. WebRTC 解码
   ↓
   HTML5 Video Element

6. Three.js VideoTexture
   ↓
   GPU 纹理 (1280×480)

7. Shader 拆分
   ↓
   左眼纹理 (采样 U: 0.0~0.5)
   右眼纹理 (采样 U: 0.5~1.0)

8. 图层系统
   ↓
   左眼相机 → 左眼屏幕 (图层 1)
   右眼相机 → 右眼屏幕 (图层 2)

9. VR 头显渲染
   ↓
   立体视觉效果 ✨
```

## 🔧 关键技术点

### 1. **瞳距（IPD）计算**
- 人眼平均瞳距：64mm
- 左眼位置 = 头部位置 - 右向量 × 32mm
- 右眼位置 = 头部位置 + 右向量 × 32mm

### 2. **四元数旋转**
- VR 头显朝向 → 四元数
- 四元数 → 旋转矩阵
- 旋转矩阵 × 方向向量 = 相机朝向

### 3. **UV 坐标映射**
- 左眼：`uv.x = uv.x * 0.5`（采样左半部分）
- 右眼：`uv.x = uv.x * 0.5 + 0.5`（采样右半部分）

### 4. **图层系统**
- 父相机必须启用所有子相机需要的图层
- 子相机独立配置可见图层
- 实现左右眼分离渲染


## 📚 相关文件

- **服务器端**：
  - `virtual-robot/robot_sim.py` - 场景设置
  - `virtual-robot/stereo_camera.py` - 双目渲染
  - `virtual-robot/webrtc_server.py` - 视频传输

- **客户端**：
  - `src/vr-scene.js` - VR 场景和图层配置
  - `src/stereo-shader.js` - Shader 拆分
  - `src/webrtc-client.js` - WebRTC 接收
  - `main.js` - 主流程控制

